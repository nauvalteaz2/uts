...
if (isset($_GET['hapus'])) {
    ...
    header("Location: Dokter.php"); // Refresh halaman setelah hapus
    exit;
}
...
if (isset($_GET['edit'])) {
    $edit_id = $_GET['edit'];
    $query = "SELECT * FROM dokter WHERE id = $edit_id";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($edit_id) {
        ...
        $query = "UPDATE dokter SET nama = '$nama', alamat = '$alamat', telepon = '$telepon' WHERE id = $edit_id";
    } else {
        // Simpan data baru jika edit_id tidak ada
        $query = "INSERT INTO dokter (nama, alamat, telepon) VALUES ('$nama', '$alamat', '$telepon')";
    }

    mysqli_query($conn, $query);
    header("Location: Dokter.php"); // Refresh halaman setelah simpan atau update
    exit;
}
...
<body>
    ...
    <div class="container mt-5">
        <h2>Dokter</h2>
        <form method="post" action="Dokter.php<?= $edit_id ? '?edit=' . $edit_id : '' ?>">
                ...
                <?php $no = 1; while ($row = mysqli_fetch_assoc($result)): ?>
                    <tr>
                        ...
                        <td>
                            ...
                            <a href="Dokter.php?hapus=<?= $row['id']; ?>" class="btn btn-danger btn-sm" onclick="return confirm('Yakin ingin menghapus data ini?');">Hapus</a>
                        </td>
                    </tr>
                <?php endwhile; ?>
            </tbody>
